...
// Small helpers (avoid relying on other parts of the file)
function _getBalancesMonth() {
  return (
    document.querySelector("#balancesMonth")?.value ||
    (window.state && window.state.currentMonth) ||
    (typeof yyyymm === "function" ? yyyymm() : "")
  );
}

function _getApiBase() {
  // Try common patterns used in this project
  try {
    if (typeof getApiBase === "function") return String(getApiBase() || "").trim();
  } catch (_) {}
  return String(
    document.querySelector("#apiBase")?.value ||
    document.querySelector("#apiBaseInput")?.value ||
    ""
  ).trim();
}

function _getAuthHeaders() {
  // If your app stores token differently, this still won’t break.
  const token =
...
async function exportBalancesCsv() {
  const ym = _getBalancesMonth();
  const base = _getApiBase();
  if (!base) throw new Error("API base is not set (top input). Click 'Use this API' first.");

  const cleanBase = base.replace(/\/+$/, "");
  const paths = [
    `/balances/export?month=${encodeURIComponent(ym)}`,
    `/dashboard/balances/export?month=${encodeURIComponent(ym)}`,
    `/balances/export.csv?month=${encodeURIComponent(ym)}`
  ];
  ...
  for (const p of paths) {
    try {
      const url = cleanBase + p;
      const r = await fetch(url, { headers });
      if (r.ok) {
        res = r;
        break;
      }
      // Capture body for debugging (often contains error message)
  ...
  if (!res) throw lastErr || new Error("Balances export failed (no endpoint succeeded).");

  // If server accidentally returns JSON, surface it instead of downloading garbage
...
function loadPaymentsSafe(...args) {
  ...
  console.warn("loadPayments() is missing — skipping Payments load");
}
